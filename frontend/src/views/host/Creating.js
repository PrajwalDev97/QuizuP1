...
class Creating extends Component {
  ...
  shuffle(array) {
    ...
    // Function to generate a secure random number between min (inclusive) and max (exclusive)
    function getRandomInt(min, max) {
      const range = max - min;
      const randomBytes = new Uint8Array(1);
      window.crypto.getRandomValues(randomBytes);
      return min + (randomBytes[0] / 255) * range;
    }

    while (0 !== currentIndex) {
      randomIndex = Math.floor(getRandomInt(0, currentIndex));
      currentIndex -= 1;
