...
const PROPERTY_HANDLERS = {
  "http-port": (config, value, fail) =>
    (config.httpPort = parsePort(value, fail)),
  "https-port": (config, value, fail) =>
    (config.httpsPort = parsePort(value, fail)),
  "https-key-file": (config, value, fail) =>
    (config.httpsKeyFile = parseFile(value, fail)),
  "https-cert-file": (config, value, fail) =>
    (config.httpsCertFile = parseFile(value, fail)),
  "static-assets-source": (config, value, fail) =>
    (config.staticAssetSource = parseStaticAssetsSource(value, fail)),
};
...
module.exports.getConfig = (configFile) => {
  ...
  const config = extractConfigurationProperties(lines, (line, message) =>
    utils.fail([failedToLoadConfigFile, `${line.trim()}`, message])
  );
  validateConfiguration(config, (message) =>
    utils.fail([failedToLoadConfigFile, message])
  );
  return config;
};
...
function createConfigurationFileAndExit(configFile) {
  ...
  fs.writeFileSync(
    configFile,
    fs.readFileSync(defaultConfigPath).toString().replace(/\r/g, ""),
    { flag: "wx" }
  );
  utils.fail([
...
//   } else {
//     return fail(
//       "The static asset source must be set to 'local', 'github', or an http(s):// URL"
//     );
//   }
// }
...
function validateConfiguration(config, fail) {
  ...
  if (
    !!config.httpsPort !== !!config.httpsKeyFile ||
    !!config.httpsKeyFile !== !!config.httpsCertFile
  ) {
    fail(
      "https-port, https-key-file and https-cert-file must all be set together"
    );
  }
}
